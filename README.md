...
- Left click recording (middle click to stop)
...
- **Record** a new routine:
  ...
  2. **Left click** anywhere to record that position.
  3. **Middle click** to stop recording.
  4. Optionally save the routine to a `.json` file for reuse.
...
Routines are saved as a JSON array of click objects:

```json
[
  { "x": 540, "y": 360, "button": "left" },
  { "x": 800, "y": 200, "button": "left" }
]
```

You can edit these files manually to adjust coordinates.

---

## Known Limitations

- Left clicks only  right click and drag are not supported
- No GUI  all interaction is via the terminal
- No per-click timing capture  a single global delay is applied to all clicks
